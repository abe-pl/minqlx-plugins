...
class afk(iouonegirlPlugin):
            ...
            for p in teams['red'] + teams['blue']:
                ...
                if prev_pos == curr_pos:
                    ...
                    if secs+interval >= self.warning and secs < self.warning:
                        self.help_warn(p)
                    if secs+interval >= self.detection and secs < self.detection:
    ...
    def help_warn(self, player):
        message = "You have been inactive for {} seconds\n^1In {} seconds you will be moving to spectators".format(self.warning, self.detection-self.warning)
        minqlx.send_server_command(player.id, "cp \"\n\n\n{}\"".format(message))

    @minqlx.next_frame
    def help_detected_print(self, player):
        self.msg("^1{} ^1has been inactive for {} seconds!".format(player.name, int(self.positions[player.steam_id][1])))
        if self.put_to_spec:
            player.put('spectator')
            self.msg("^1{} ^1was moved to spectators".format(player.name))
